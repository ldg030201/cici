...
```sh
npm test           # node --test test/*.test.js — 의존성 0
...
├─ claude-core.js        Claude 확장 id·저장 키·값 판정 규칙. 역시 공유
...
├─ _locales/             8개 로케일(ko·en·zh_CN·pt·ja·es·de·fr). en 이 기본(default_locale)
└─ lib/
   ├─ leveldb-core.js  ┐
   ├─ snappy.js        │ src/ 의 자동 생성 복사본. 직접 고치지 말 것 —
   ├─ claude-core.js   ┘ build:ext 가 만들고, 테스트가 바이트 단위로 비교한다
   ├─ fileurl.js        file:// 바이트 소스 + Chromium 디렉터리 리스팅 파서
   ├─ locate.js         프로필 열거 + nonce 왕복
   └─ read.js           프로필 → bridgeDeviceId / bridgeDisplayName
```

`extension/lib` 의 `leveldb-core.js`·`snappy.js`·`claude-core.js` 는 자동 생성물이다.
