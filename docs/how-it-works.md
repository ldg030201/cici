...
이 왕복은 브라우저 세션당 사실상 한 번이다. 확인된 답(자기 프로필 폴더)과 마지막
검사 결과는 메모리 전용 `chrome.storage.session` 에 캐시되므로, 같은 세션에서 팝업을
다시 열면 직전 결과를 즉시 그리고 재검사만 뒤에서 돌린다 — 같은 팝업이 다른
프로필에서 돌 수는 없기 때문에 자기 탐지는 다시 할 필요가 없다. 캐시는 디스크에
기록되지 않고 브라우저를 닫으면 사라진다.
